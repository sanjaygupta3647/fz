...
<?php 
		...
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "Copy":
					...
					break;
				case "delete":
					$cms->db_query("delete from #_faq where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
				case "Inactive":
					$cms->db_query("update #_faq set status = 'Inactive' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_faq set status = 'Active' where pid in ($str_adm_ids)");
	...
	$columns = "select * ";
	$sql = " from #_faq where store_user_id='".$_SESSION[uid]."' and 1 $cond ";
...
?>
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  class="data-tbl">
    <tr class="t-hdr">
      ...
      <td width="15%" align="center"><?=$adm->orders('Qsn',true)?></td>  
	   <td width="15%" align="center"><?=$adm->orders('Question For',true)?></td> 
