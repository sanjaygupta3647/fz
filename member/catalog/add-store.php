...
if($cms->is_post_back()){ 
		$allkeys = "";
 		$_POST[store_url] = $cms->subdomain(trim(addslashes($_POST['title']))) ;  
    ...
    <div class="div-tbl">
      ...
      <div class="title"  id="innertit">
       <h2 class="bradcrumb"><?php
		if($mode=='add' && $id!=''){?>
			...
			<a href="/member/catalog" rel="v:url" property="v:title">Add Store</a> » 
        ...
        <table width="100%" border="0" align="left" cellpadding="2" cellspacing="1"  class="frm-tbl2">
		   ...
		   <tr  class="grey_">
            ...
            <td width="75%"> 
				...
				?>
                <select name="storekey[]" multiple style="height:100px;" required="required" class="txt medium" id="storekey"  title="Storekey"> 
	  ...
	  ?>
		  ...
		  <tr  class="grey_">
            ...
            <td width="77%"><input required="required"  name="pincode" title="pincode" lang="R"  type="text" class="txt medium" 
			id="Address" value="<?=$pincode?>" /></td>
          </tr>
          ...
          <?php } ?>
          <tr>
            <td valign="top" class="label"> Your Logo:</td>
		  ...
		  <?php $brands=$cms->db_query("select brand_id  from #_request_brand where store_user_id =".$_SESSION[uid]." and status = 'Active'");
				$tcnt = mysql_num_rows($brands); 
				if($tcnt){?>
		   <tr  class="grey_">
            <td width="23%" class="label">Your Brand Logo:</td>
            <td width="77%"><?php  while($stRes=$cms->db_fetch_array($brands)){   
										...
										$getTitle = $cms->getSingleresult("select title from #_store_detail where store_user_id = '".$stRes[brand_id]."' and status = 'Active'");?>  
										 ...
										 <input type="radio" name="image" value="<?=$image?>" /><img src="<?=SITE_PATH?>uploaded_files/orginal/<?=$image?>" width="50">(<?=$getTitle?>)
			 <?php }   ?> </td>  
          </tr>  <?php } ?>
		 ...
		 <tr  class="grey_">
            <td width="23%" class="label">Own Domain Name if any</td>
            <td width="77%"><input   name="store_domain" title="store domain"   type="text" class="txt medium" 
			id="store_domain" value="<?=$store_domain?>" /></td>
          </tr>
