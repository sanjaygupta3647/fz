...
<?php } ?>
        <div class="brdcm" id="hed-tit">Sells Product</div>
        <div class="unvrl-btn">  
...
$getproduct= $cms->db_query("select prod_id from #_barnds_product where brand_id = '".$_SESSION[uid]."'");
	...
	$columns ="SELECT * ";
	$sql =" from #_barnds_product where brand_id='".$_SESSION[uid]."' $cond group by store_user_id ";  
    ...
    <h1></h1> 
      ...
      <div class="title"  id="innertit">
		...
		if($mode=='add' && $id!=''){?>
			...
			<a href="/member/catalog" rel="v:url" property="v:title">Product main sells</a> » 
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  class="data-tbl">
          <tr class="t-hdr">
            ...
            <td width="37%" align="center"><?=$adm->orders('Store Name',true)?></td>
			 <td width="27%" align="center"><?=$adm->orders('Total Sale',true)?></td>  
            <td width="27%" align="center"><?=$adm->orders('Product Sale',true)?></td>  
          </tr>
          ...
          <tr <?=$adm->even_odd($nums)?>>
			...
			<?php  
			 $amountTotals=0;
			 $amountTotal =$cms->db_query("select amount from #_orders_detail where proid in  (".implode(',',$prod_arr).") and  store_id='$store_user_id'");
		     while ($data=$cms->db_fetch_array($amountTotal)){ 
					$amountTotals=$amountTotals + $data[amount];
			 }?>
			 <td align="center">Rs. <?=$amountTotals?></td>
             <td align="center">
			<?php  
			 $tot =$cms->db_query("select * from #_orders_detail where proid in  (".implode(',',$prod_arr).") and  store_id='$store_user_id'");
		     $cnt = mysql_num_rows($tot);?>
			 <a href="<?=SITE_PATH_MEM.CPAGE."/product-sub-sells.php?storeId=".$store_user_id?>">Product Sale(<?=$cnt?>)</a>  
			 </td> 
          </tr>
...
if(dayfrom){
 ...
 var red = "<?=SITE_PATH_MEM.CPAGE?>/product-main-sells.php"+ur;
   window.location = red; 
});
