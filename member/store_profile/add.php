...
if($cms->is_post_back()){ 
	...
	$_POST[url] = $adm->baseurl($title);  
	if($updateid){	
		$uids =  $cms->sqlquery("rs","contact",$_POST,'pid',$updateid);
	...
	$cms->db_query("update #_contact set `body` = '".$_POST[body]."' where `pid` in ($uids)");
...
?>
  ...
  <table width="100%" border="0" align="left" cellpadding="4" cellspacing="1" class="frm-tbl2">
	...
	<tr>
	  <td valign="top" class="label">Query:</td>
	  <td valign="top"><?=$query?></td>
    </tr>
	<tr>
	  <td valign="top" class="label">Status:</td>
	  <td valign="top"><?=($status)?'Viewed':'Not Viewed'?></td>
    </tr>
