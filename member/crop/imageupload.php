<?php  
	...
	$err = ""; 
     ...
     $pathimg = ($_GET[image])?$_GET[image]:"product";
	 if($delid){
	...
	if($_POST[upload]){
			...
			{
				 $redir = SITE_PATH_MEM.CPAGE."/imageupload.php?imgid=upimg&image=".$_GET[image];
			...
			$thumb = UP_FILES_FS_PATH."/thumb"; 

			if($_FILES[file1][name]){ 
				$ext=end(explode('.',$_FILES[file1][name]));					
				$imgname=rand().time().'.'.$ext;  
				$bool = move_uploaded_file($_FILES[file1][tmp_name],$path.'/'.$imgname);  
				$err = 0;
				if($ext=='png' || $ext =='PNG' || $ext=='gif' || $ext =='GIF') $err = 1;
				if(!$err)$cms->make_thumb($path."/".$imgname, $thumb."/".$imgname, 200,180);  
				...
				$img = $path."/".$imgname;  
				$redir = SITE_PATH_MEM.CPAGE."/imageupload.php?imgid=".$_GET[imgid]."&image=".$_GET[image];
				echo "<script>self.location='".$redir."'</script>";  
			}
		}  
	}
...
<title>Fizzkart image upload and crop</title>
   ...
   <script type="text/javascript" src="js/jquery.imgareaselect-0.3.min.js"></script>
    ...
    <link rel="stylesheet" href="<?=SITE_PATH_MEM?>crop/css/style.css" />
    <link rel="stylesheet" href="<?=SITE_PATH_MEM?>css/style.css" />
</head> 
<body>
...
<div class="wrap-box">
   ...
   <?php
	...
	$columns = "select * ";
	$sql = " from #_images where path = '".$_GET[image]."' and store_user_id = '".$_SESSION[uid]."' ";
	if($_GET[name]!=""){
  ...
  <div class="upload radius">
   <div class="section"> 
	 ...
	 ?>     <div id="BrowserVisible">
               ...
               <img style="position:absolute; display:none;" src="<?=SITE_PATH_MEM?>images/ajax-loader.gif" height="32" id="ajaximg" /> 
              </div>
   ...
   <div class="contens radius"> 
    ...
    <tr <?=$adm->even_odd($nums)?>>
		...
		<td align="left" colspan="5">
		 <p> 
      <input type="submit" onClick="return confirm('Are you sure to delete this image?');" name="delete" value="delete" /></p> </td> 
		  
	</tr>
    <tr class="tbl-head tbl-head155"> 
    <th>COUNT</th>
    <th><input type="checkbox"  id="imgid" /></th>
     <th> IMAGE</th> <th>INSERT</th> <th>URL</th></tr>
    ...
    <td align="center"><input type="text" value="<?=SITE_PATH?>uploaded_files/orginal/<?=$image?>"/></td> 
      
    </tr>
    <?php $i++; } }} else{?>
	
	<tr><td colspan="6"> No Image Uploaded!</td></tr>
	<? }?>
    </table>
    </form>
