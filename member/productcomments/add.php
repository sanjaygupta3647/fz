...
if(isset($id)){
	...
	@extract($arrAdmin);
	$pname=$cms->getSingleresult("select title from #_products_user where pid='".$prod_id."' ");
	$name=$cms->getSingleresult("select name from #_store_user where pid='".$_SESSION[uid]."' "); 
	$name=str_replace(" ",'',$name);
	$urls="http://".$name.".fizzkart.com"."/detail/".$pname."/".$prod_id;
}
?>
 ...
 <tr class="grey_">
	  ...
	  <td ><?=ucfirst($type)?>&nbsp;Name <a href="<?=($urls)?$urls:'#'?>" target="_blank"><?=ucfirst($cms->getSingleresult("select title from #_products_user where pid='".$prod_id."' "));?></a> 
	  <?php $img=$cms->getSingleresult("select image1 from #_products_user where pid='".$prod_id."' ");  ?>
        <a href="<?=($urls)?$urls:'#'?>" target="_blank"><img src="<?=$cms->getImageSrc($img)?> " width="80" height="45"  alt="" align="left"/> </a></td>
    </tr> 
	<tr  class="grey_">
      <td width="25%" valign="top" class="label">Title:</td>
      <td width="75%"><textarea name="title" cols="80" rows="1" lang="R" title="Title" id="Title"><?=$title?></textarea></td>
    </tr>  
	<tr>
