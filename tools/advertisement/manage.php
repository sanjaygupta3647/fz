...
<?php 
	...
	if($action=='del'){
 		$cms->db_query("delete from #_advertise where pid in ($id)");
		...
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "Copy":
					...
					break;
				case "delete":
					$cms->db_query("delete from #_advertise where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
				case "Inactive":
					$cms->db_query("update #_advertise set status = 'Inactive' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_advertise set status = 'Active' where pid in ($str_adm_ids)");
	...
	$columns = "select * ";
	$sql = " from #_advertise where 1 $cond ";
...
?>
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  class="data-tbl">
    <tr class="t-hdr">
      ...
      <td width="15%" align="center"><?=$adm->orders('Link',true)?></td>
	  <td width="15%" align="center"><?=$adm->orders('Place',true)?></td>
    ...
    <td align="center"><img src="<?=SITE_PATH?>uploaded_files/orginal/<?=$image?>" width="100" /></td> 
    <td align="center"><?=$title?></td>
    <td align="center"><?=$linkurl?></td> 
	<td align="center"><?=ucwords($place)?></td> 
