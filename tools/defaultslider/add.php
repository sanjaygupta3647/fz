...
if($cms->is_post_back()){  
$numc = mysql_num_rows($cms->db_query("select * from #_slider where slider_type='0'"));
$_POST[store_id]=0;
	if($pid){ 
	//echo  $_POST[image1],"</br>", $_POST[color];die();
	  if($_POST[image1]){
...
?>
 ...
 <input name="discounttype" type="radio" id="radio1"   value="color" <?=($ext!='.png' || $ext!='.jpg' || $ext!='.jpeg')?'checked':''?>>Color	&nbsp;&nbsp; 
 <input name="discounttype" type="radio" id="radio2"   value="image1" <?=($ext=='.png' || $ext=='.jpg' || $ext=='.jpeg')?'checked':''?>> Image</td>
 </tr>   
	 <tr  class="grey_ color">
	  <td class="label">Background Color</td> 
	  <td><input name="color" class="color" value="" lang="R"></td>
	</tr>
	 
	<?php if($image  and is_file($_SERVER['DOCUMENT_ROOT'].SITE_SUB_PATH."uploaded_files/orginal/".$image)==true){?>
          <tr class="grey_ image1">
      ...
      <td width="75%">
      <input type="text" name="image1" value="" class="txt medium" id="upimg" />
    ...
    <tr>
	<?php if($side_image  and is_file($_SERVER['DOCUMENT_ROOT'].SITE_SUB_PATH."uploaded_files/orginal/".$side_image)==true){?>
          <tr>
            <td valign="top" class="label">&nbsp;</td>
            <td valign="top"><img src="<?=SITE_PATH?>uploaded_files/orginal/<?=$side_image?>" width="100"> &nbsp;&nbsp;
            </td>
          </tr>
          <?php } ?>
     ...
     <tr  class="grey_">
	  ...
	  <td><input name="title" type="text"  value="<?=$title?>" lang="R" class="txt medium" /></td>
	</tr>
	
	   <tr  class="grey_">
	  <td class="label">Cotaints:</td> 
	  <td>  
	  <textarea   cols="20"  name="contents" title="Contents"  class="txt medium" lang="R"/>
	  <?=$contents?>
	</textarea></td>
	</tr>   
   <tr  class="grey_">
  ...
  <script type="text/javascript">
   $(document).ready(function(){
	...
	$(".image1").hide(); 
	<?php
		if($ext!='.png' || $ext!='.jpg' || $ext!='.jpeg') {?> $(".color").show(); $(".image1").hide(); <?php }  
		if($ext=='.png' || $ext=='.jpg' || $ext=='.jpeg') {?> $(".image1").show(); $(".color").hide(); <?php } ?>
