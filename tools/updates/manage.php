...
<?php 
	if($action=='del'){
		$cms->db_query("delete from #_updates where pid in ($id)");
	...
	if($cms->is_post_back()){
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "delete":
					$cms->db_query("delete from #_updates where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
				case "Inactive":
					$cms->db_query("update #_updates set status = 'Inactive' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_updates set status = 'Active' where pid in ($str_adm_ids)");
	...
	$columns = "select * ";
	$sql = " from #_updates where store_user_id = '0' ";
...
<div class="internal-data">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  class="data-tbl">
    <tr class="t-hdr">
      ...
      <td width="18%" align="center"><?=$adm->orders('Sub Title',true)?></td>  
	  <td width="18%" align="center"><?=$adm->orders('Add Time',true)?></td>
    ...
    <td align="center"><?=ucwords(str_replace('-',' ',$title))?></td> 
