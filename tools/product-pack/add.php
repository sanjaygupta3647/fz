...
<?php 
 ...
 if($cms->is_post_back() and !$_POST[search]){  
	 ...
	 if($id){
		$cms->sqlquery("rs","product_pack",$_POST,'pid',$updateid); 
		$adm->sessset('Record has been updated', 's');
	}else{
		$_POST[submitdate] = time();
		$cms->sqlquery("rs","product_pack",$_POST);
...
if(isset($id)){
	$rsAdmin=$cms->db_query("select * from #_product_pack where pid='".$id."'");
...
?>
  ...
  <table width="100%" border="0" align="left" cellpadding="4" cellspacing="1" class="frm-tbl2">     
    <tr>
      <td width="25%"  class="label">Pack Name: </td>
      <td width="75%"><input type="text" name="pack_name"  lang="R" title="pack" class="txt medium" value="<?=$pack_name?>" style="width:100px;" placeholder="Pack Name" />
	   </td>
    </tr>
	 <tr>
