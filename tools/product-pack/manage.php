...
<?php 
	...
	if($cms->is_post_back() and !$_POST[search]){
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "delete":
					$cms->db_query("delete from #_product_pack where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
			   case "Inactive":
					$cms->db_query("update #_product_pack set status = 'Inactive' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_product_pack set status = 'Active' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Active', 's');
					break;
				case "Block":
					$cms->db_query("update #_product_pack set status = 'Block' where pid in ($str_adm_ids)");
     ...
     if($_GET[action]=='del'){
		$cms->db_query("delete from #_product_pack where pid='".$_GET[id]."'");
		$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
	...
	if($search){
		$pagesize = 1000;
		if($_GET[product_pack]){
			$cond .=" and product_pack =".$_GET[product_pack];
		}
		if($_GET[amount]){
	...
	$columns = "select * ";
	$sql = " from #_product_pack where 1 $cond  ";
...
?>
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  class="data-tbl">
    <tr class="t-hdr">
      ...
      <td width="10%" align="center"><?=$adm->orders('Pack Name',true)?></td> 
