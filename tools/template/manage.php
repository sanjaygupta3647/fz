...
<?php  
	...
	if($cms->is_post_back() and !$_POST[search]){
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "delete":
					$cms->db_query("delete from #_template where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
				case "Inactive":
					$cms->db_query("update #_template set status = 'Inactive' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_template set status = 'Active' where pid in ($str_adm_ids)");
	...
	$start = intval($start);
	$columns = "select * ";
	$sql = " from #_template where store_id ='0' ";
...
<tr class="t-hdr">
  ...
  <td width="8%" align="center"><?=$adm->orders('Subject',true)?></td>  
