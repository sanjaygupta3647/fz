<? if($items[3])
	{  
      ...
      $qry_var1 = $cms->db_query("SELECT voucherCode,amount,status FROM #_gift_voucher where voucherCode ='".$cms->encryptcode($items[3])."'");
	 ...
	 if($status=$res1['status']=='Inactive'){
	   ...
	   <p>Discount for Coupon = Rs.<?=$res1['amount']?> /-</p>  
	   <p>Payable Amount = Rs.<?=$dueAmount?></p> 
	   <?php
