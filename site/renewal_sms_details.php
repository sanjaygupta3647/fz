...
?></p>
 ...
 <form action="renewal_sms_sumit" method="post" autocomplete="off" >
		 <input type="hidden" name="plan_id" value="<?=$plan_id?>" />
<div class="renewal_main3">
<p><strong>Select Your SMS Plan:</strong>  </p>
</div>
<div class="renewal_main4"><?php
$qry = $cms->db_query("select * from fz_sms_pack where user_id =0 "); ?>
	<select name="sms_pack" id="sms_pack" required>
	<?php while($res = $cms->db_fetch_array($qry)){ ?>
					    <option value="<?=$res[pid]?>"><?=$res[sms_pack]?>(<?=$res[qty]?> SMS) and Amount <?=$res[amount]?></option>
	 <?php } ?>
	</select>		
