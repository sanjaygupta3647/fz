...
<div  class="main_matter_area"> 
  <div class="main_text_area1">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="30" align="center" class="enq_text_fild2">Brand Store(<?=str_replace('-',' ',$items[1])?>)</td>
      </tr>
      <tr>
        <td align="center" valign="top">
			<?php 
			$plans = $cms->getPlanIdByBrand($items[2]);
			 
			$search =  "select pid,title,plan_id,image,theme from #_store_detail where plan_id in ('".implode(',',$plans)."') "; 
			/*"select pid,body1, title,image1 from #_products_user where brand_id = ".$items[2]; */
			...
			?>
			<tr>
				<td height="30" align="center" class="enq_text_fild2">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr><td align="left" width="20%" ><a style="margin-left:20px;"
				 href="<?=SITE_PATH?><?=($re_visited['theme'])?$re_visited['theme']:'domain'?>/<?=$adm->baseurl($re_visited['title'])?>" target="_blank" ><img src="<?=$img?>" height="60" width="60" /></a></td>
				  <td align="left"><h5><?=stripslashes($re_visited['title'])?></h5>
				 <?php /*?><?=substr(stripslashes(strip_tags(trim($re_visited['body1']))),0,200)?><?php */?> </td></tr> 
				</table>
				</td>
			</tr> 
			<tr>
