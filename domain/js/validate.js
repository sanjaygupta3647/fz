...
function formvalid3(frmObj)
{var i,p,q,nm,test,num,min,max,errors='',args=formvalid.arguments;j=0;var regEmail=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;var regBlank=/[^\s]/;var regAlphaNum=/^([a-zA-Z0-9_.]+)$/;var regAlpha=/^([a-zA-Z ]+)$/;var regDate=/^([0-9_]+-[0-9][0-9]+-[0-9][0-9]+)$/;var formtype=frmObj.elements;var checked='';for(i=0;i<(formtype.length);i++)
{mesg=formtype[i].title;test=formtype[i].lang;val=formtype[i];val_new=formtype[i];val_new.style.backgroundColor='';if(val)
{nm=mesg;val=val.value;if(regBlank.test(val))
{if(test.indexOf('isAlphaNum')!=-1)
{result=trim(val);if(result.length==0){errors+='- '+nm+' is required.\n';}else{if(!regAlphaNum.test(val))
{errors+='- '+nm+': Only Alpha Numeric and "_" Chars Allowed.\n';val_new.style.backgroundColor='#e2e2e2';}}}
else if(test.indexOf('isAlpha')!=-1)
{result=trim(val);if(result.length==0){errors+='- '+nm+' is required.\n';}else{if(!regAlpha.test(val))
{errors+='- '+nm+': Only Alpha Chars Allowed.\n';val_new.style.backgroundColor='#e2e2e2';}}}
else if(test.indexOf('RisCheck')!=-1)
{if(!val_new.checked){errors+='- '+nm+' is Not Checked.\n';}}
else if(test.indexOf('isDate2')!=-1)
{errors+=isDate1(val,'/');}
else if(test.indexOf('isUsername')!=-1)
{errors+=isUsername(val);}
...
{p=val.indexOf('@');s=val.indexOf('.');if(p<1||p==(val.length-1))
{errors+='- '+nm+' must contain an e-mail Address.\n';val_new.style.backgroundColor='#e2e2e2';}
else if(!regEmail.test(val))
{errors+='- '+nm+' must contain a valid e-mail Address.\n';val_new.style.backgroundColor='#e2e2e2';}}
else if(test.indexOf('isAmount')!=-1)
{if(parseFloat(val)<parseFloat(val_new.getAttribute('maxlength')))
{errors+='- You cannot request transfers for amounts under Ã‚Â£'+val_new.getAttribute('maxlength')+'.\n';val_new.style.backgroundColor='#e2e2e2';}}
else if(test.indexOf('isNaN')!=-1)
{if(isNaN(val))
{errors+='- '+nm+' must contain a number.\n';val_new.style.backgroundColor='#e2e2e2';}}
else if(test.indexOf('isMax')!=-1)
...
{p=val.indexOf('http://');s=val.indexOf('.');if(p<0||p==(val.length-1))
{errors+='- '+nm+' must be valid URL e.g. http://www.abc.com\n';val_new.style.backgroundColor='#e2e2e2';}
else if(s<p||s==(val.length-1))
{errors+='- '+nm+' must be valid URL e.g. http://www.abc.com\n';val_new.style.backgroundColor='#e2e2e2';}}else if(test.indexOf('isChar')!=-1)
{var first_char;first_char=val.charAt(0);if(first_char==0||first_char==1||first_char==2||first_char==3||first_char==4||first_char==5||first_char==6||first_char==7||first_char==8||first_char==9){errors+='- '+nm+' must starts with  a char.\n';val_new.style.backgroundColor='#e2e2e2';}}
else if(test.charAt(0)=='R')
{result=trim(val);if(result.length==0){errors+='- '+nm+' is required.\n';val_new.style.backgroundColor='#e2e2e2';}}
else if(test.indexOf('isEqual')!=-1)
{equal_obj_val=test.substring(7,test.indexOf(":"));mesg_string=test.substring((test.indexOf(":")+1));if(val!=formtype[equal_obj_val].value)
{errors+='- '+nm+' must be same to '+mesg_string+'.\n';formtype[equal_obj_val].style.backgroundColor='#e2e2e2';val_new.style.backgroundColor='#e2e2e2';}}}
else if(test.charAt(0)=='R'){errors+='- '+nm+' is required.\n';val_new.style.backgroundColor='#e2e2e2';}}
if(errors!="")
{if(j<=0)
{focusitem=formtype[i];j++;}}}
if(errors)
{alert('The following error(s) occurred:\n\n'+errors);focusitem.focus();return false;}
else
return true;}