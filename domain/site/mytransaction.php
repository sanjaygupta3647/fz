<?php  
$metaTitle = $cms->getSingleresult("select meta_title from #_meta_info where url='my-transaction' and store_user_id = '$current_store_user_id'");
$metaIntro = $cms->getSingleresult("select meta_description from #_meta_info where url='my-transaction' and store_user_id = '$current_store_user_id'");
$metaKeyword = $cms->getSingleresult("select meta_keyword from #_meta_info where url='my-transaction' and store_user_id = '$current_store_user_id'"); 
...
<div  class="profile_tabs">
 <?php include "user-tabs.php";  ?> 
  <div class="selected">
   <?php 
	 $ordercheck=$cms->db_query("select * from #_order_summary where uid = '".$_SESSION['userid']."' and store_id = '$current_store_id' ");
	 if(mysql_num_rows($ordercheck)){?> <table border="0" width="100%" class="CSSTableGenerator"> 
	 <tr><th align="left">Order ID</th><th align="left">Amount</th><th align="left">Date</th><th align="left">Status</th></tr> 
	 <?php
	 while($res =$cms->db_fetch_array($ordercheck)){?> <? 
	if($cms->getSingleresult("select paynet from #_order_summary where orderid='".$res[orderid]."'")){	 
	 ?>
	 <tr>
	 <td>
	 ...
	 <td><?=$cms->price_format($cms->getSingleresult("select paynet from #_order_summary where orderid='".$res[orderid]."'"))?></td>
	  ...
	  <?php 
	   ...
	   if(mysql_num_rows($check)){?>
		...
		while($res2 = $cms->db_fetch_array($check))
					    ...
					    $imgpro  = SITE_PATH_M."uploaded_files/orginal/no-img.gif";
						if(file_exists('../uploaded_files/orginal/'.$img) && $img!="")
						{
							$imgpro  = SITE_PATH_M."uploaded_files/orginal/".$img;
						}
						?>
