<?php
$metaTitle = $cms->getSingleresult("select title from #_store_detail where store_user_id = '".$items[1]."'")." brand product(s)"; 
$metaIntro = $cms->getSingleresult("select title from #_store_detail where store_user_id = '".$items[1]."'")." brand product(s)";
$metaKeyword = $cms->getSingleresult("select meta_keyword from #_meta_info where url='brand-page' and store_user_id = '$current_store_user_id'"); 
$ssid = session_id();
...
$prod_arr[] = 0;
$catsIds[] = 0;
$brandProds = $cms->db_query("select prod_id,cat_id from #_barnds_product where store_user_id='$current_store_user_id' and  brand_id ='".$items[1]."'  ");			
...
$_SESSION[catid]=$items[2]; 
  ...
  <div class="main_text_brdr_none"><?php
    include_once "brand-left-panel.php"; 
    ...
    <div class="apparel_paging3" id="content">
      <div style="width:100%" class="main_text_areain_apparel">
        <h3><?=$cms->removeSlash($cms->getSingleresult("select title from #_store_detail where store_user_id = '".$items[1]."'"))?> Brand Product(s)<span>(<?=$count?> Product<?=($cntbrand>1)?'s':''?>)</span></h3>
      </div>
      <?php  
	if($count){ 
